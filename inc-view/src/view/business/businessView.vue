...
<!--            <addForm :call="search" :data="scope.row" size="mini" title="编辑"/>-->
              <el-button type="danger":disabled="scope.row.owner != null" @click="seize(scope.$index,scope.row)" size="mini">抢占</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      seize(index, row) {
        let that = this;
        this.$confirm('确认抢占该商机吗?', '提示', {type: 'warning'}).then(() => {
          that.loading = true;
          api.seize({id:row.id}, (result) => {
            that.loading = false;
            if (result && result.code == '0') {
              that.$message.success({showClose: true, message: '抢占成功！', duration: 1500});
              that.search();
            } else {
